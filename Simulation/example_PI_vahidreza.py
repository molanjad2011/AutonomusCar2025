...
def compute_right_lane_point(lines, y_target, right_lane_point, image_width):
    ...
    res = (right_lane_point+ 2*np.mean(right_x))/3 # در نهایت میانگین میگیریم
    return res
...
pid_right = PID(Kp=3.05, Ki=0.000000025, Kd=11)
pid_left = PID(Kp=3.1, Ki=0, Kd=11.3)
...
def process_image_and_compute_steering(image, lane_point, offset=50, obstT:bool = False):
    ...
    diffrence = pid_left.update(cte) if obstT else pid_right.update(cte)
