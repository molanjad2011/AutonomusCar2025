...
def compute_right_lane_point(lines, y_target, right_lane_point, image_width):
    ...
    res = (right_lane_point+ 3*np.mean(right_x))/4 # در نهایت میانگین میگیریم
    return res

# pid = PID(Kp=7.6, Ki=0.000000025, Kd=14.8)
pid = PID(Kp=4.05, Ki=0, Kd=5.7)
...
def process_image_and_compute_steering(image, lane_point, offset=50):
    ...
    y_target = int(height * 0.9)
    lane_point = compute_right_lane_point(lines, y_target, lane_point, width)
...
try:
    ...
    while True:
        ...
        if counter > 4:
            ...
            steering, lane_point = process_image_and_compute_steering(image, lane_point, -100)
            ...
            if abs_steering > 37:
                ...
                burst_counter = 2  # تعداد فریم‌هایی که سرعت باید -100 باشه
                car.setSpeed(-100)
            else:
                car.setSpeed(100)'''
